...
async def connect():  # connect to the server
  """
  For connecting to the websocket server.

  Takes no parameters and returns nothing.
  """
  global config
  global ws
  # Check if the server address is already set in config.json, if not, ask for a server
...
async def auth():
  """
  For authneticating with the websocket server.
  ...
  # formats the auth messages
  JSONAuth = json.dumps(
      {"type": "auth", "username": username, "password": hashedPass})
...
async def is_command(message):
  """
  For checking if a message contains a command.

  Takes a message as a parameter and returns a bool if the message is a command.
  """
...
async def send_message(message):
  """
  For sending a message to the websocket server.

  Takes a message as a parameter and returns nothing.
  """
  global ws
  JSONOutput = json.dumps(
      {"type": "message", "message": message.strip("\n")})
...
async def parse_command(message):
  ...
  if message.split()[0] == "|w":  # private messages
    if message.split()[1] in users_list and not message.split()[1] == username:
      JSONOutput = json.dumps({"type": "direct_message", "recipient": message.split()[
                              1], "message": " ".join(message.split()[2:])})
      await ws.send(JSONOutput)
  ...
  if not message == previousMsg:
    ...
    if m_type == "message":
      ...
      if message['new_account']:
        ...
        print(auth_stat.format(
            "Successfully authenticated with an existing account."))
      elif not message['new_account'] and not message['success']:
        print(auth_stat.format(
            "There was an error authenticating, please check your password."))
...
try:
  ...
  loop.run_until_complete(asyncio.gather(receive_message(), input_message()))
